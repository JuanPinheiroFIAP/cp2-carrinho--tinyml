...
1. Grave `src/arduino/tinyml_b_preprocessamento` na ESP32 (placa **DOIT ESP32 DEVKIT V1**, pacote ESP32) e **feche o Monitor Serial**, senão a porta fica ocupada.
2. Instale a dependência: `pip install pyserial` (ou `poetry install`).
...
7. Grave `src/arduino/tinyml_c_modelo` e rode `python testar_modelo.py --segundos 40 --saida teste_modelo_placa.txt` movendo o objeto.
